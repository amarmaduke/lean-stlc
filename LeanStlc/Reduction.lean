...
| fst : Red (.fst (.pair a b)) a
| snd : Red (.snd (.pair a b)) b
| pair1 : Red a a' -> Red (.pair a b) (.pair a' b)
| pair2 : Red b b' -> Red (.pair a b) (.pair a b')
...
| case_inl :
  ...
  ParRed a a' ->
  ParRed b b' ->
  ParRed (.case A (.inl t) a b) a'
| case_inr :
  ParRed t t' ->
  ParRed a a' ->
  ParRed b b' ->
  ParRed (.case A (.inr t) a b) b'
| case :
  ParRed t t' ->
  ParRed a a' ->
  ParRed b b' ->
  ParRed (.case A t a b) (.case A t' a' b')
| fst :
  ParRed a a' ->
  ParRed b b' ->
